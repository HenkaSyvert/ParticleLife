...
var music_scales: Array[Array] = [penta_scale, jap_penta_scale, hicaz_scale, major_scale, melodic_minor_scale,
...
func _ready() -> void:
	...
	for i: int in range(num_notes):
		...
		var t: Transform3D = Transform3D(Basis(), note_strings[i] * particle_life.universe_radius / 2)
			...
			if d > 0.9:
				...
				if note_timers[i] < 0:
					...
					if enable_sound:
						...
						(%HarpSampler as SamplerInstrument).play_note(notes[i % notes.size()], starting_octave + i / notes.size())
